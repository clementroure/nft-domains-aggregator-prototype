...
const checkAllUD = (domainName: string, setResults: any) => {
    ...
    const UDVerifyDisponibility = () => {
      ...
      .then(async (data) => {
        ...
        for (let i =0; i< 9; i++){
          ...
          console.log("TEST: " + data.exact[i].availability)
          if(data.exact[i].availability == false){
            ...
            .then((data) => {
              ...
              // const isMatic = data.properties.records
              // console.log(isMatic)
